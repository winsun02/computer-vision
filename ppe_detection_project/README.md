...
## 13. Chạy thuần trên Google Colab từ branch `main`
...
Notebook này được thiết kế cho workflow **Colab + GitHub**, không dùng luồng upload project local/zip. Các bước chạy khuyến nghị:

1. Đảm bảo code đã được push lên branch `main` của repo GitHub bạn đang làm.
2. Mở Google Colab và upload/open file `notebooks/ppe_detection_colab.ipynb`.
3. Chọn `Runtime -> Change runtime type -> GPU` để train nhanh hơn.
4. Trong cell đầu tiên, điền `GIT_REPO_URL` bằng link GitHub repo bạn đang làm. Giữ `BRANCH = "main"`. Notebook sẽ tự `git clone --branch main` repo đó và tìm thư mục `ppe_detection_project`.

Ví dụ cấu hình trong notebook:

```python
GIT_REPO_URL = "https://github.com/<username>/computer-vision.git"
BRANCH = "main"
```

5. Chạy cell cài đặt dependencies bằng `pip install -r requirements.txt`.
6. Upload file `kaggle.json` khi notebook yêu cầu, hoặc cấu hình Kaggle credential trước trong Colab.
7. Chạy cell chuẩn bị dataset:
...
8. Chạy cell train/evaluate/detect ảnh. Notebook sẽ dùng GPU nếu `torch.cuda.is_available()` trả về `True`. Cell train truyền `--project` bằng đường dẫn tuyệt đối `PROJECT_DIR / "runs/train"` và tự tìm `BEST_MODEL` mới nhất, nên cell evaluate không bị lỗi sai đường dẫn `best.pt`.
9. Cuối notebook có cell zip thư mục `runs` để tải kết quả về máy.

Nếu muốn chạy lại phiên bản mới nhất, hãy push commit mới lên branch `main`, sau đó Runtime → Restart runtime và chạy lại notebook từ đầu.
...
### Colab báo `Model weights not found` khi Evaluate

Nguyên nhân thường gặp là notebook cũ train model vào một thư mục khác, ví dụ `runs/detect/runs/train/...`, nhưng cell evaluate lại tìm `runs/train/ppe_yolo/weights/best.pt`.

Cách sửa:

1. Pull/cập nhật notebook mới nhất từ branch `main`.
2. Runtime → Restart runtime trên Colab.
3. Chạy lại từ đầu. Notebook mới truyền đường dẫn train tuyệt đối qua `--project` và tự gán `BEST_MODEL` bằng file `best.pt` mới nhất trong `runs/train`.
